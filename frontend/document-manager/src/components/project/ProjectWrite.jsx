// frontend/document-manager/src/components/project/ProjectWrite.jsx
import { useState, useEffect } from 'react';
import { PenTool, Loader2, Save, Download, AlertCircle } from 'lucide-react';
...
const ProjectWrite = ({ 
  ...
  const generateWrite = async () => {
    ...
    if (selectedData.buckets.length === 0 && selectedData.tables.length === 0 && selectedData.brainstormVersions.length === 0) {
    ...
    try {
      // For now, use the same brainstorm API endpoint
      // In a real implementation, this would be a separate /api/write endpoint
      const response = await fetch('/api/brainstorm', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            dataSourcesCount: selectedData.buckets.length + selectedData.tables.length + selectedData.brainstormVersions.length
          }
        })
      });

      if (!response.ok) {
        throw new Error('Write request failed');
      }
  ...
  const exportAllVersions = () => {
    ...
    a.href = url;
    a.download = `${project.name}_write_versions.csv`;
    ...
    <div className="space-y-8">
      <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-stone-900">AI Writing</h2>
            <p className="text-stone-600">Generate structured content from your research</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <PromptBuilder
              ...
              buckets={buckets}
              tables={tables}
              brainstormVersions={brainstormVersions}
            ...
            <button
              ...
              className="w-full bg-purple-500 text-white px-6 py-4 rounded-lg hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium flex items-center justify-center gap-2"
            >
              {isGenerating ? (
                <>
          ...
          <div className="space-y-6">
            {currentResult && (
              <div className="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden">
                <div className="p-4 border-b border-stone-100 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-stone-800">Generated Content</h3>
                </div>
                <div className="p-6">
                  <ResultsDisplay content={currentResult} type="write" />
                </div>
              </div>
            )}
            ...
            {selectedVersion && !currentResult && (
              <div className="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden">
                ...
                <div className="p-6">
                  <ResultsDisplay content={selectedVersion.result} type="write" />
                </div>
              </div>
            )}
          </div>
      ...
      {saveDialog.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-stone-800 mb-4">Save Write Version</h3>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 bg-purple-500 text-white px-4 py-2 rounded-lg hover:bg-purple-600 disabled:opacity-50 transition-colors"
              >
                Save Version
              </button>
            </div>
